...
темы и главный экран без прокрутки при повороте и крупном шрифте. Скриншоты и
отчёты тестов загружаются отдельно. Секреты подписи нужны для обоих режимов.
UI Automator работает в отдельном модуле и процессе `smoke`, сохраняя обычную
релизную оптимизацию и путь запуска приложения.
